...
本目录用于承载 Fenlie 当前阶段的融资、路演、尽调与 pilot 推进文档。
...
- 对外叙事包
- investor / advisor / pilot 对话支持包
- data room 与 Notion 入口素材层
...
2. **Canonical outreach flow**
   - `2026-03-24-fenlie-unified-outreach-pack-and-notion-index.md`
3. **Canonical diligence hub**
   - `2026-03-24-fenlie-data-room-index-and-request-list.md`

如果以后新增 investor 文档：

- 先补正文
- 再补 `investor-pack-index`
- 再判断是否需要补 `unified-outreach-pack-and-notion-index`
- 如涉及对外发送动作，再补 `minimal-send-pack-manifest`
- 如涉及尽调入口，再补 `data-room-index-and-request-list`
...
2. 不写收益承诺
3. 不写虚构客户、收入、ARR、估值
4. 不把 OI / orderflow / triad sidecar 写成成熟主执行信号
5. 不把 execution bridge 写成成熟规模化 execution SaaS
6. 不把当前状态表述成“稳定接管大规模 live capital”
...
- base round discussion：`$2.2M–$2.8M`
- runway：`18–24 个月`
...
- `2026-03-24-fenlie-tam-sam-som-and-3y-financial-model.md`
...
- 统一使用：`YYYY-MM-DD-fenlie-<topic>.md`
- 不在本目录放无日期、无主题、无口径归属的临时文档

### 变更顺序

修改 investor 材料时，优先顺序固定为：
...
1. 新文件是否被 `investor-pack-index` 收录
2. 是否破坏 `$2.2M–$2.8M / 18–24 个月` 统一口径
3. 是否误写 execution 成熟度
4. `diligence-readiness-checklist` 是否需要同步

---

## 8. 当前残余缺口

截至 2026-03-24，本目录仍有两类未闭环项：

1. **正式版 deck / PDF 成品** 尚未落地
2. **外部共享载体**（Notion / data room / zip 包）尚未生成成品，只完成了文稿和结构索引

因此当前状态应表述为：

- investor narrative：已具备
- pack governance：已具备
- final deliverable packaging：进行中
...
1. 产出正式版 deck / PDF 成品稿
2. 按 `deck-production-and-export-checklist` 生成会前 / 路演 / 会后版
3. 生成最小发送包清单（可直接发投资人）
4. 生成 data room 初版目录
5. 视需要迁移到 Notion / 共享目录
