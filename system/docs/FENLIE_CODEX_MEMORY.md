...
5. `challenge_pair` must derive from source-owned chain; pair slug naming is `challenger_vs_baseline`.
...
- Memory tree + contracts under `system/docs/memory/` is the long-term durable entry, while this file remains compatibility bootstrap only.
- Dashboard/public/internal surface contracts have dedicated validation paths; avoid encoding transient runtime state in legacy memory.
- Exit/risk and hold-side research flows should use single-entry refresh runners and source-owned artifacts.
- Remote live guard diagnostics keep their own source artifacts/handoff chain; this legacy file must not carry live state.
