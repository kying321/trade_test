...
- 主代理默认读取：`AGENTS.md` → `FENLIE_CODEX_MEMORY.md` → `memory/MEMORY_INDEX.md` → `memory/SHORTLIST.md` → （Task 2 落地后）相关 contract。
- 子代理默认读取：`memory/SHORTLIST.md` → （Task 2 落地后）与任务直接相关的 contract，必要时再补 `memory/DURABLE_RULES.md`。
- 人类默认序列：`memory/SHORTLIST.md` → `memory/DURABLE_RULES.md` → 争议时再退回到（Task 2 落地后的）具体 contract。

## Task-to-Document Map
- 快速恢复及身份指引：`memory/SHORTLIST.md`
- 关于长期规范与可变边界：`memory/DURABLE_RULES.md`
- 深层 contract（source、live、research 等）：Task 2 落地后的 `memory/contracts/` 目录各章
- 总体导航与文件布局：本文件
