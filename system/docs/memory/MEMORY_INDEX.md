...
- 长期规则、深层契约移至 `memory/DURABLE_RULES.md` 与 `memory/contracts/`。
- 旧的 `FENLIE_CODEX_MEMORY.md` 保持作为兼容引导 shim，不再承载临时状态。
...
└── contracts/
    ...
    ├── OPERATOR_HANDOFF.md（后续扩展）
    └── FEEDBACK_LEARNING.md（后续扩展）
```
