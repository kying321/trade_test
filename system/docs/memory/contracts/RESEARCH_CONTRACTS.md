...
## Retained Mainline
- 当前 durable 主线保持：`ETHUSDT / 15m / single-symbol / breakout-pullback`。
...
- baseline retained 且 canonical handoff active 的语义是：基线继续作为有效裁决头被保留，并可被执行链消费。
- baseline retained 不应被误读为“尚未决策”或“自动等待下一次晋升”。
...
- 三者冲突时，以 source-owned handoff + blocker 的显式控制字段优先。
...
- consumer 只能消费这三个字段，不得在外层 summary 再造并行版本。
