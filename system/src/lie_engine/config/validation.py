...
def validate_settings(settings: SystemSettings) -> dict[str, Any]:
    ...
    if "crypto_factor_failover_enabled" in val and not isinstance(val.get("crypto_factor_failover_enabled"), bool):
        issues.append(ValidationIssue("error", "validation.crypto_factor_failover_enabled", "必须是布尔值"))
    if "crypto_factor_failover_window_runs" in val and _as_int(val.get("crypto_factor_failover_window_runs", 0)) < 1:
        issues.append(ValidationIssue("error", "validation.crypto_factor_failover_window_runs", "必须 >= 1"))
    if "crypto_factor_failover_freeze_days" in val and _as_int(val.get("crypto_factor_failover_freeze_days", 0)) < 1:
        issues.append(ValidationIssue("error", "validation.crypto_factor_failover_freeze_days", "必须 >= 1"))
    if "crypto_factor_failover_lookback_days" in val and _as_int(val.get("crypto_factor_failover_lookback_days", 0)) < 1:
        issues.append(ValidationIssue("error", "validation.crypto_factor_failover_lookback_days", "必须 >= 1"))
    if "crypto_factor_failover_profit_factor_min" in val and _as_float(val.get("crypto_factor_failover_profit_factor_min", 0.0)) <= 0.0:
        issues.append(ValidationIssue("error", "validation.crypto_factor_failover_profit_factor_min", "必须 > 0"))
    if "crypto_factor_failover_window_runs" in val and "crypto_factor_failover_lookback_days" in val:
        wr = _as_int(val.get("crypto_factor_failover_window_runs", 0))
        lb = _as_int(val.get("crypto_factor_failover_lookback_days", 0))
        if lb < wr:
            issues.append(ValidationIssue("error", "validation.crypto_factor_failover_lookback_days", "必须 >= validation.crypto_factor_failover_window_runs"))
    if "crypto_execution_friction_guard_enabled" in val and not isinstance(val.get("crypto_execution_friction_guard_enabled"), bool):
        issues.append(ValidationIssue("error", "validation.crypto_execution_friction_guard_enabled", "必须是布尔值"))
    if "crypto_execution_friction_p95_bps_max" in val and _as_float(val.get("crypto_execution_friction_p95_bps_max", 0.0)) <= 0.0:
        issues.append(ValidationIssue("error", "validation.crypto_execution_friction_p95_bps_max", "必须 > 0"))
    if "crypto_execution_friction_depth_floor_usdt" in val and _as_float(val.get("crypto_execution_friction_depth_floor_usdt", 0.0)) <= 0.0:
        issues.append(ValidationIssue("error", "validation.crypto_execution_friction_depth_floor_usdt", "必须 > 0"))
    if "crypto_execution_friction_target_notional_usdt" in val and _as_float(val.get("crypto_execution_friction_target_notional_usdt", 0.0)) <= 0.0:
        issues.append(ValidationIssue("error", "validation.crypto_execution_friction_target_notional_usdt", "必须 > 0"))
    if "crypto_execution_friction_artifact_max_age_days" in val and _as_int(val.get("crypto_execution_friction_artifact_max_age_days", -1)) < 0:
        issues.append(ValidationIssue("error", "validation.crypto_execution_friction_artifact_max_age_days", "必须 >= 0"))
    ...
    if not isinstance(core, list) or not core:
        ...
        for i, item in enumerate(core):
            ...
            if not (EQUITY_RE.match(sym) or FUTURE_RE.match(sym) or BINANCE_PAIR_RE.match(sym)):
