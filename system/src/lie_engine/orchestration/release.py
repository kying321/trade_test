...
class ReleaseOrchestrator:
    ...
    @staticmethod
    def _trim_text_tail(text: str, *, max_lines: int = 20, max_chars: int = 2000) -> str:
        if not text:
            return ""
    ...
    def _guard_loop_baseline_rollback_preflight(self, *, as_of: date) -> dict[str, Any]:
        val = self.settings.validation if isinstance(self.settings.validation, dict) else {}
        enabled = self._safe_bool(val.get("ops_guard_loop_baseline_rollback_drill_enabled", True), True)
        source_mode = str(val.get("broker_snapshot_source_mode", "paper_engine")).strip().lower() or "paper_engine"
        paper_only = self._safe_bool(val.get("ops_guard_loop_baseline_rollback_drill_paper_only", True), True)
        paper_mode = source_mode.startswith("paper_engine")
        if not enabled:
            return {
                "enabled": False,
                "active": False,
        ...
        if paper_only and not paper_mode:
            return {
                "enabled": True,
                "active": False,
                "passed": True,
                "status": "skipped",
                "reason": "paper_mode_only",
                "paper_mode": bool(paper_mode),
                "source_mode": source_mode,
            }

        root_dir = Path(__file__).resolve().parents[3]
        script_path = root_dir / "scripts" / "baseline_promotion_rollback_drill.sh"
        review_dir = self.output_dir / "review"
        review_dir.mkdir(parents=True, exist_ok=True)
        promotion_file = review_dir / f"{as_of.isoformat()}_baseline_promotion.json"
        if not promotion_file.exists():
            candidates = sorted(review_dir.glob("*_baseline_promotion.json"), key=lambda p: p.stat().st_mtime, reverse=True)
            promotion_file = candidates[0] if candidates else Path("")
        if not promotion_file.exists():
            return {
                "enabled": True,
                "active": True,
        ...
        try:
            ...
            for line in stdout.splitlines():
                txt = line.strip()
                if txt.startswith("artifact_json="):
                    artifact_json = txt.split("=", 1)[1].strip()
            passed = bool(proc.returncode == 0 and "PASSED baseline rollback drill" in stdout)
            return {
                "enabled": True,
                "active": True,
                "passed": bool(passed),
                "status": "passed" if passed else "failed",
                "reason": "ok" if passed else f"returncode_{int(proc.returncode)}",
                "paper_mode": bool(paper_mode),
                "source_mode": source_mode,
                "script": str(script_path),
                "promotion_file": str(promotion_file),
                ...
                "stdout_tail": self._trim_text_tail(stdout, max_lines=12, max_chars=1200),
                "stderr_tail": self._trim_text_tail(stderr, max_lines=12, max_chars=1200),
            }
            ...
            return {
                ...
                "promotion_file": str(promotion_file),
                "returncode": 124,
                "stdout_tail": self._trim_text_tail(str(exc.stdout or ""), max_lines=12, max_chars=1200),
                "stderr_tail": self._trim_text_tail(str(exc.stderr or ""), max_lines=12, max_chars=1200),
            }
    ...
    def _artifact_governance_effective_policies(self) -> dict[str, dict[str, Any]]:
        val = self.settings.validation if isinstance(self.settings.validation, dict) else {}
        raw_profiles = (
            val.get("ops_artifact_governance_profiles", {})
            if isinstance(val.get("ops_artifact_governance_profiles", {}), dict)
        ...
        profile_names = set(self.ARTIFACT_GOVERNANCE_DEFAULTS.keys()) | set(self.ARTIFACT_GOVERNANCE_LEGACY_KEYS.keys())
        profile_names |= set(str(k).strip() for k in raw_profiles.keys() if str(k).strip())
        out: dict[str, dict[str, Any]] = {}
        for profile_name in sorted(profile_names):
            legacy_defaults = self._artifact_governance_legacy_defaults(profile_name=profile_name)
            policy = self._artifact_governance_profile(
                profile_name=profile_name,
                fallback_retention_days=int(legacy_defaults.get("retention_days", 30)),
                fallback_checksum_index_enabled=bool(legacy_defaults.get("checksum_index_enabled", True)),
            )
            out[str(profile_name)] = {
                "json_glob": str(policy.get("json_glob", "")),
                "md_glob": str(policy.get("md_glob", "")),
                "checksum_index_filename": str(policy.get("checksum_index_filename", "")),
    ...
    def _promote_artifact_governance_baseline(
        ...
        *,
        ...
        val = self.settings.validation if isinstance(self.settings.validation, dict) else {}
        if not self._safe_bool(val.get("review_loop_baseline_promotion_enabled", True), True):
            return {
                "enabled": False,
                "promoted": False,
                "reason": "disabled",
        ...
        baseline_profiles = self._artifact_governance_effective_policies()
        base_dir = self.output_dir / "artifacts" / "baselines" / "artifact_governance"
        history_dir = base_dir / "history"
        history_dir.mkdir(parents=True, exist_ok=True)
        active_path = base_dir / "active_baseline.yaml"
        previous_active: dict[str, Any] = {}
        if active_path.exists():
            previous_active = self.load_json_safely(active_path)
        previous_anchor = str(previous_active.get("snapshot_path", "")).strip()
        if previous_anchor and not Path(previous_anchor).exists():
            previous_anchor = ""
        fallback_anchor = str(review.rollback_anchor).strip()
        rollback_anchor = previous_anchor or fallback_anchor
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        snapshot_path = history_dir / f"{as_of.isoformat()}_r{int(round_no):02d}_{stamp}.yaml"
        payload = {
            "enabled": True,
            "promoted": True,
            "reason": "ok",
            "active_path": str(active_path),
            "history_path": str(snapshot_path),
            "snapshot_path": str(snapshot_path),
            "rollback_anchor": rollback_anchor,
            "profiles_count": int(len(baseline_profiles)),
            "promoted_at": datetime.now().isoformat(),
            "as_of": as_of.isoformat(),
            "round": int(round_no),
            "profiles": baseline_profiles,
        }
        write_markdown(snapshot_path, yaml.safe_dump(payload, allow_unicode=True, sort_keys=False))
        write_markdown(active_path, yaml.safe_dump(payload, allow_unicode=True, sort_keys=False))
        review_dir = self.output_dir / "review"
        review_dir.mkdir(parents=True, exist_ok=True)
        write_json(review_dir / f"{as_of.isoformat()}_baseline_promotion.json", payload)
        return payload
    ...
    def review_until_pass(self, as_of: date, max_rounds: int = 3) -> dict[str, Any]:
        ...
        for i in range(int(max_rounds)):
            ...
            if ok:
                baseline_promotion: dict[str, Any]
                try:
                    ...
                    write_json(
                        self.output_dir / "review" / f"{as_of.isoformat()}_baseline_promotion.json",
                        baseline_promotion,
                    )
    ...
    def run_review_cycle(self, as_of: date, max_rounds: int = 2, ops_window_days: int | None = None) -> dict[str, Any]:
        ...
        baseline_rollback_drill = self._guard_loop_baseline_rollback_preflight(as_of=as_of)
        review_loop = self.review_until_pass(as_of=as_of, max_rounds=max_rounds)
        gate = self.gate_report(as_of=as_of, run_tests=False, run_review_if_missing=False)
        ops = self.ops_report(
            as_of=as_of,
            window_days=ops_days,
        )
        health = self.health_check(as_of, True)
        return {
            "date": as_of.isoformat(),
            ...
            "health": health,
            "heartbeat": {
                "guard_loop_preflight": {
                    "baseline_rollback_drill": baseline_rollback_drill,
                }
            },
        }
