...
def prune_artifacts(
    ...
    *,
    ...
    candidates = sorted(
        review_dir.glob("*_remote_live_noaf_probe*.json"),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a local artifact from a remote RestrictAddressFamilies=none probe."
    )
...
def main() -> int:
    ...
    out: dict[str, Any] = {
        ...
        "probe": probe_payload,
        "artifact_status_label": "noaf-compatible" if ok else status,
        "artifact_label": None,
        "artifact_tags": [],
        "artifact": None,
        "checksum": None,
    ...
    out["artifact_label"] = f"remote-live-noaf-probe:{suffix}"
    out["artifact_tags"] = ["remote-live", "noaf-probe", suffix]

    review_dir.mkdir(parents=True, exist_ok=True)
    stamp = now_ts.astimezone(dt.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    artifact_path = review_dir / f"{stamp}_remote_live_noaf_probe.json"
    checksum_path = review_dir / f"{stamp}_remote_live_noaf_probe_checksum.json"
