...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sync repo system/ into the local OpenClaw PI workspace without touching runtime output/ state."
    )
    parser.add_argument(
        "--source-root",
        default=str(Path(__file__).resolve().parents[1]),
        help="Repo system root to copy from.",
    )
    parser.add_argument(
        "--target-root",
        default=str(Path.home() / ".openclaw" / "workspaces" / "pi" / "fenlie-system"),
        help="Local PI workspace system root to copy into.",
    )
    parser.add_argument(
        "--backup-keep",
        type=int,
        default=DEFAULT_BACKUP_KEEP,
        help="Keep at most this many workspace_sync backups under target output/backups/.",
    )
    parser.add_argument(
        "--backup-max-age-hours",
        type=int,
        default=DEFAULT_BACKUP_MAX_AGE_HOURS,
        help="Delete workspace_sync backups older than this age.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Report the sync plan without mutating files.",
    )
    parser.add_argument(
        "--no-backup",
        action="store_true",
        help="Disable overwrite backups for this sync run.",
    )
    parser.add_argument(
        "--pulse-lock-path",
        default="",
        help="Override run-halfhour-pulse mutex path. Defaults to <target-root>/output/state/run_halfhour_pulse.lock.",
    )
    parser.add_argument(
        "--lock-timeout-sec",
        type=float,
        default=DEFAULT_LOCK_TIMEOUT_SEC,
        help="Mutex acquisition timeout for non-dry-run syncs.",
    )
    parser.add_argument(
        "--lock-retry-sec",
        type=float,
        default=DEFAULT_LOCK_RETRY_SEC,
        help="Mutex acquisition retry interval for non-dry-run syncs.",
    )
    return parser
...
def main() -> int:
    ...
    pulse_lock_path = (
        ...
        else (target_root / "output" / "state" / "run_halfhour_pulse.lock").resolve()
    )

    out: dict[str, object] = {
        "action": "sync_local_pi_workspace",
        "ok": False,
        "changed": False,
        "dry_run": dry_run,
    ...
    try:
        ...
        if not (source_root / "config.yaml").exists():
            raise FileNotFoundError(f"source_config_missing:{source_root / 'config.yaml'}")
