...
def normalize_event(row: dict[str, Any], *, now_text: str) -> dict[str, Any]:
    ...
    feedback_id = safe_text(sanitized.get("feedback_id")) or f"manual_{now_text}"
    event = {
        "feedback_id": feedback_id,
        "created_at_utc": safe_text(sanitized.get("created_at_utc")) or now_text,
        "source": safe_text(sanitized.get("source")) or "manual",
...
def publish_manual_payload(*, review_dir: Path, payload: Any, now_text: str | None = None) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "status": "ok",
        "action": "publish_conversation_feedback_manual_internal",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Append sanitized internal manual conversation feedback rows.")
