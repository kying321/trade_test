...
def prune_artifacts(
    ...
    *,
    ...
    candidates = sorted(
        review_dir.glob("*_remote_live_procsubset_probe*.json"),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a local artifact from a remote ProcSubset probe.")
...
def main() -> int:
    ...
    out: dict[str, Any] = {
        "action": "build_remote_live_procsubset_probe",
        "ok": ok,
        "status": status,
        ...
        "probe": probe_payload,
        "artifact_status_label": "procsubset-compatible" if ok else status,
        "artifact_label": None,
        "artifact_tags": [],
        "artifact": None,
        "checksum": None,
    ...
    out["artifact_label"] = f"remote-live-procsubset-probe:{suffix}"
    out["artifact_tags"] = ["remote-live", "procsubset-probe", suffix]

    review_dir.mkdir(parents=True, exist_ok=True)
    stamp = now_ts.astimezone(dt.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    artifact_path = review_dir / f"{stamp}_remote_live_procsubset_probe.json"
    checksum_path = review_dir / f"{stamp}_remote_live_procsubset_probe_checksum.json"
