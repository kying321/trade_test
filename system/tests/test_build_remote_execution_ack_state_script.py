from __future__ import annotations

import json
import subprocess
from pathlib import Path


SCRIPT_PATH = (
    Path(__file__).resolve().parents[1]
    / "scripts"
    / "build_remote_execution_ack_state.py"
)


def _write_json(path: Path, payload: dict[str, object]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def test_build_remote_execution_ack_state_records_learning_only_ack_when_policy_stays_shadow_only(
    tmp_path: Path,
) -> None:
    review_dir = tmp_path / "review"
    _write_json(
        review_dir / "20260315T103500Z_remote_orderflow_policy_state.json",
        {
            "policy_brief": "shadow_policy_learning_only:SOLUSDT:queued_wait_trade_readiness:downrank_guardian_blocked_route",
            "policy_status": "shadow_policy_learning_only",
            "policy_decision": "accept_shadow_learning_only",
            "policy_recommendation": "continue_shadow_learning_until_guardian_clear",
            "route_symbol": "SOLUSDT",
            "route_action": "deprioritize_flow",
            "remote_market": "portfolio_margin_um",
            "blocker_detail": "guardian blocked",
        },
    )
    _write_json(
        review_dir / "20260315T102110Z_openclaw_orderflow_executor_state.json",
        {
            "executor_brief": "shadow_guarded_executor_ready:SOLUSDT:queued_wait_trade_readiness:portfolio_margin_um",
            "executor_status": "shadow_guarded_executor_ready",
        },
    )
    _write_json(
        review_dir / "20260315T102105Z_openclaw_orderflow_executor_heartbeat.json",
        {
            "executor_brief": "shadow_guarded_idle:SOLUSDT:queued_wait_trade_readiness:portfolio_margin_um",
            "executor_status": "shadow_guarded_idle",
            "executor_action": "observe_only_no_transport",
            "idempotency_key": "entry-1",
            "idempotency_status": "fresh_intent_observed",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )
    _write_json(
        review_dir / "20260315T102000Z_remote_execution_journal.json",
        {
            "journal_brief": "queued_wait_trade_readiness:SOLUSDT:deprioritize_flow:portfolio_margin_um | blocked:risk_guard | not_attempted_wait_trade_readiness",
            "journal_status": "intent_logged_guardian_blocked",
            "last_entry_key": "entry-1",
            "fill_status": "no_fill_execution_not_attempted",
            "blocker_detail": "guardian blocked",
        },
    )

    proc = subprocess.run(
        [
            "python3",
            str(SCRIPT_PATH),
            "--review-dir",
            str(review_dir),
            "--now",
            "2026-03-15T10:35:30Z",
        ],
        text=True,
        capture_output=True,
        check=False,
    )

    assert proc.returncode == 0, proc.stderr
    payload = json.loads(proc.stdout)
    assert payload["ack_status"] == "shadow_learning_ack_recorded"
    assert payload["ack_decision"] == "record_learning_without_transport"
    assert payload["transport_state"] == "not_sent_learning_only"
    assert payload["journal_entry_key"] == "entry-1"
    assert Path(str(payload["artifact"])).name == "20260315T103530Z_remote_execution_ack_state.json"


def test_build_remote_execution_ack_state_waits_for_transport_when_policy_accepts(
    tmp_path: Path,
) -> None:
    review_dir = tmp_path / "review"
    _write_json(
        review_dir / "20260315T103500Z_remote_orderflow_policy_state.json",
        {
            "policy_brief": "shadow_policy_candidate_ready:SOLUSDT:queued_ticket_ready:feedback_cleared",
            "policy_status": "shadow_policy_candidate_ready",
            "policy_decision": "accept_shadow_candidate",
            "policy_recommendation": "allow_shadow_candidate_to_progress",
            "route_symbol": "SOLUSDT",
            "route_action": "long_breakout_pullback",
            "remote_market": "portfolio_margin_um",
        },
    )
    _write_json(
        review_dir / "20260315T102110Z_openclaw_orderflow_executor_state.json",
        {
            "executor_brief": "shadow_guarded_executor_ready:SOLUSDT:queued_ticket_ready:portfolio_margin_um",
            "executor_status": "shadow_guarded_executor_ready",
        },
    )
    _write_json(
        review_dir / "20260315T102105Z_openclaw_orderflow_executor_heartbeat.json",
        {
            "executor_brief": "shadow_guarded_ticket_ready:SOLUSDT:queued_ticket_ready:portfolio_margin_um",
            "executor_status": "shadow_guarded_ticket_ready",
            "executor_action": "shadow_ready_wait_guard_clear",
            "idempotency_key": "entry-2",
            "idempotency_status": "fresh_intent_observed",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )
    _write_json(
        review_dir / "20260315T102000Z_remote_execution_journal.json",
        {
            "journal_brief": "queued_ticket_ready:SOLUSDT:long_breakout_pullback:portfolio_margin_um | ready:policy_clear | not_attempted",
            "journal_status": "intent_logged_ready",
            "last_entry_key": "entry-2",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )

    proc = subprocess.run(
        [
            "python3",
            str(SCRIPT_PATH),
            "--review-dir",
            str(review_dir),
            "--now",
            "2026-03-15T10:35:31Z",
        ],
        text=True,
        capture_output=True,
        check=False,
    )

    assert proc.returncode == 0, proc.stderr
    payload = json.loads(proc.stdout)
    assert payload["ack_status"] == "shadow_transport_pending_ack"
    assert payload["ack_decision"] == "wait_for_transport_ack"
    assert payload["transport_state"] == "ready_not_sent_guard_clear_pending"


def test_build_remote_execution_ack_state_records_runtime_boundary_block_without_transport(
    tmp_path: Path,
) -> None:
    review_dir = tmp_path / "review"
    _write_json(
        review_dir / "20260315T103500Z_remote_orderflow_policy_state.json",
        {
            "policy_brief": "shadow_policy_runtime_boundary_blocked:SOLUSDT:queued_execution_contract_blocked:route_quality_ok",
            "policy_status": "shadow_policy_runtime_boundary_blocked",
            "policy_decision": "hold_requested_runtime_promotion",
            "policy_recommendation": "implement_requested_runtime_before_nonshadow_promotion",
            "route_symbol": "SOLUSDT",
            "route_action": "seed_ticket",
            "remote_market": "spot",
            "blocker_detail": "requested runtime missing",
        },
    )
    _write_json(
        review_dir / "20260315T102110Z_openclaw_orderflow_executor_state.json",
        {
            "executor_brief": "executor_runtime_boundary_blocked:SOLUSDT:queued_execution_contract_blocked:spot",
            "executor_status": "executor_runtime_boundary_blocked",
            "runtime_boundary_status": "requested_runtime_not_implemented",
        },
    )
    _write_json(
        review_dir / "20260315T102105Z_openclaw_orderflow_executor_heartbeat.json",
        {
            "executor_brief": "requested_mode_not_implemented:SOLUSDT:queued_execution_contract_blocked:spot:spot_live_guarded",
            "executor_status": "requested_mode_not_implemented",
            "executor_action": "idle_requested_mode_not_implemented",
            "idempotency_key": "entry-3",
            "idempotency_status": "fresh_intent_observed",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )
    _write_json(
        review_dir / "20260315T102000Z_remote_execution_journal.json",
        {
            "journal_brief": "queued_execution_contract_blocked:SOLUSDT:seed_ticket:spot | contract:promotion_requested | not_attempted_execution_contract_blocked",
            "journal_status": "intent_logged_guardian_blocked",
            "last_entry_key": "entry-3",
            "fill_status": "no_fill_execution_not_attempted",
            "blocker_detail": "requested runtime missing",
        },
    )

    proc = subprocess.run(
        [
            "python3",
            str(SCRIPT_PATH),
            "--review-dir",
            str(review_dir),
            "--now",
            "2026-03-15T10:35:32Z",
        ],
        text=True,
        capture_output=True,
        check=False,
    )

    assert proc.returncode == 0, proc.stderr
    payload = json.loads(proc.stdout)
    assert payload["ack_status"] == "shadow_runtime_boundary_ack_recorded"
    assert payload["ack_decision"] == "record_runtime_boundary_block_without_transport"
    assert payload["transport_state"] == "not_sent_runtime_boundary_blocked"
    assert payload["executor_runtime_boundary_status"] == "requested_runtime_not_implemented"


def test_build_remote_execution_ack_state_records_guarded_probe_completion_before_runtime_boundary(
    tmp_path: Path,
) -> None:
    review_dir = tmp_path / "review"
    _write_json(
        review_dir / "20260315T103500Z_remote_orderflow_policy_state.json",
        {
            "policy_brief": "shadow_policy_runtime_boundary_blocked:SOLUSDT:queued_execution_contract_blocked:route_quality_ok",
            "policy_status": "shadow_policy_runtime_boundary_blocked",
            "policy_decision": "hold_requested_runtime_promotion",
            "policy_recommendation": "implement_requested_runtime_before_nonshadow_promotion",
            "route_symbol": "SOLUSDT",
            "route_action": "seed_ticket",
            "remote_market": "spot",
            "blocker_detail": "requested runtime missing",
        },
    )
    _write_json(
        review_dir / "20260315T102110Z_openclaw_orderflow_executor_state.json",
        {
            "executor_brief": "spot_live_guarded_probe_completed:SOLUSDT:queued_execution_contract_blocked:spot",
            "executor_status": "spot_live_guarded_probe_completed",
            "runtime_boundary_status": "requested_runtime_not_implemented",
            "guarded_exec_probe_status": "probe_completed",
            "guarded_exec_probe_artifact": "/tmp/probe.json",
        },
    )
    _write_json(
        review_dir / "20260315T102105Z_openclaw_orderflow_executor_heartbeat.json",
        {
            "executor_brief": "spot_live_guarded_probe_completed:SOLUSDT:queued_execution_contract_blocked:spot",
            "executor_status": "spot_live_guarded_probe_completed",
            "executor_action": "guarded_probe_completed",
            "guarded_exec_probe_status": "probe_completed",
            "guarded_exec_probe_artifact": "/tmp/probe.json",
            "idempotency_key": "entry-4",
            "idempotency_status": "fresh_intent_observed",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )
    _write_json(
        review_dir / "20260315T102000Z_remote_execution_journal.json",
        {
            "journal_brief": "queued_execution_contract_blocked:SOLUSDT:seed_ticket:spot | contract:promotion_requested | probe_completed",
            "journal_status": "intent_logged_guardian_blocked",
            "last_entry_key": "entry-4",
            "fill_status": "no_fill_execution_not_attempted",
            "blocker_detail": "requested runtime missing",
        },
    )

    proc = subprocess.run(
        [
            "python3",
            str(SCRIPT_PATH),
            "--review-dir",
            str(review_dir),
            "--now",
            "2026-03-15T10:35:33Z",
        ],
        text=True,
        capture_output=True,
        check=False,
    )

    assert proc.returncode == 0, proc.stderr
    payload = json.loads(proc.stdout)
    assert payload["ack_status"] == "shadow_guarded_probe_ack_recorded"
    assert payload["ack_decision"] == "record_guarded_probe_without_live_transport"
    assert payload["transport_state"] == "probe_completed_no_live_send"
    assert payload["guarded_exec_probe_status"] == "probe_completed"
    assert payload["guarded_exec_probe_artifact"] == "/tmp/probe.json"


def test_build_remote_execution_ack_state_records_guarded_probe_candidate_without_transport(
    tmp_path: Path,
) -> None:
    review_dir = tmp_path / "review"
    _write_json(
        review_dir / "20260315T103500Z_remote_orderflow_policy_state.json",
        {
            "policy_brief": "shadow_policy_learning_only:SOLUSDT:queued_ticket_blocked:ticket_not_allowed",
            "policy_status": "shadow_policy_learning_only",
            "policy_decision": "accept_shadow_learning_only",
            "policy_recommendation": "continue_shadow_learning_until_guardian_clear",
            "route_symbol": "SOLUSDT",
            "route_action": "consider_refresh_before_promotion",
            "remote_market": "spot",
        },
    )
    _write_json(
        review_dir / "20260315T102110Z_openclaw_orderflow_executor_state.json",
        {
            "executor_brief": "spot_live_guarded_probe_capable:SOLUSDT:queued_ticket_blocked:spot",
            "executor_status": "spot_live_guarded_probe_capable",
            "runtime_boundary_status": "guarded_probe_only_runtime",
            "guarded_exec_probe_status": "ticket_not_allowed",
        },
    )
    _write_json(
        review_dir / "20260315T102105Z_openclaw_orderflow_executor_heartbeat.json",
        {
            "executor_brief": "spot_live_guarded_probe_capable:SOLUSDT:queued_ticket_blocked:spot",
            "executor_status": "spot_live_guarded_probe_capable",
            "executor_action": "idle_guarded_probe_only_runtime",
            "executor_runtime_boundary_status": "guarded_probe_only_runtime",
            "guarded_exec_probe_status": "ticket_not_allowed",
            "idempotency_key": "entry-probe-candidate-1",
            "idempotency_status": "fresh_intent_observed",
            "fill_status": "no_fill_execution_not_attempted",
        },
    )
    _write_json(
        review_dir / "20260315T102000Z_remote_execution_journal.json",
        {
            "journal_brief": "queued_ticket_blocked:SOLUSDT:consider_refresh_before_promotion:spot | blocked:ticket_missing:no_actionable_ticket | no_probe",
            "journal_status": "intent_logged_guardian_blocked",
            "last_entry_key": "entry-probe-candidate-1",
            "fill_status": "no_fill_execution_not_attempted",
            "blocker_detail": "ticket not allowed",
        },
    )

    proc = subprocess.run(
        [
            "python3",
            str(SCRIPT_PATH),
            "--review-dir",
            str(review_dir),
            "--now",
            "2026-03-15T10:35:34Z",
        ],
        text=True,
        capture_output=True,
        check=False,
    )

    assert proc.returncode == 0, proc.stderr
    payload = json.loads(proc.stdout)
    assert payload["ack_status"] == "shadow_guarded_probe_candidate_ack_recorded"
    assert payload["ack_decision"] == "record_guarded_probe_candidate_without_transport"
    assert payload["transport_state"] == "probe_candidate_blocked_no_live_send"
    assert payload["guarded_exec_probe_status"] == "ticket_not_allowed"
