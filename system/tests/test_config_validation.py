...
class ConfigValidationTests(unittest.TestCase):
    ...
    def test_validate_settings_detects_errors(self) -> None:
        settings = SystemSettings(
            raw={
                ...
                "validation": {
                    ...
                    "crypto_factor_failover_enabled": "yes",
                    "crypto_factor_failover_window_runs": 0,
                    "crypto_factor_failover_profit_factor_min": 0.0,
                    "crypto_factor_failover_freeze_days": 0,
                    "crypto_factor_failover_lookback_days": 0,
                    "crypto_execution_friction_guard_enabled": "yes",
                    "crypto_execution_friction_p95_bps_max": 0.0,
                    "crypto_execution_friction_depth_floor_usdt": 0.0,
                    "crypto_execution_friction_target_notional_usdt": 0.0,
                    "crypto_execution_friction_artifact_max_age_days": -1,
        ...
        self.assertIn("validation.crypto_factor_failover_enabled", paths)
        self.assertIn("validation.crypto_factor_failover_window_runs", paths)
        self.assertIn("validation.crypto_factor_failover_profit_factor_min", paths)
        self.assertIn("validation.crypto_factor_failover_freeze_days", paths)
        self.assertIn("validation.crypto_factor_failover_lookback_days", paths)
        self.assertIn("validation.crypto_execution_friction_guard_enabled", paths)
        self.assertIn("validation.crypto_execution_friction_p95_bps_max", paths)
        self.assertIn("validation.crypto_execution_friction_depth_floor_usdt", paths)
        self.assertIn("validation.crypto_execution_friction_target_notional_usdt", paths)
        self.assertIn("validation.crypto_execution_friction_artifact_max_age_days", paths)
    ...
    def test_validate_settings_allows_binance_universe_symbol(self) -> None:
        project_root = Path(__file__).resolve().parents[1]
        raw = yaml.safe_load((project_root / "config.yaml").read_text(encoding="utf-8"))
        raw["data"]["provider_profile"] = "tushare_binance_hybrid"
        raw["universe"]["core"] = [{"symbol": "BTCUSDT", "asset_class": "cash", "theme": "主流币"}]
        settings = SystemSettings(raw=raw)
        out = validate_settings(settings)
