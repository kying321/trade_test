...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "build_remote_live_ipdeny_probe.py"
)
...
def test_build_remote_live_ipdeny_probe_reads_probe_payload(tmp_path: Path) -> None:
    ...
    assert payload["artifact_status_label"] == "ipdeny-compatible"
    assert payload["probe"]["status"] == "compatible"
    assert Path(str(payload["artifact"])).exists()
    assert Path(str(payload["checksum"])).exists()


def test_build_remote_live_ipdeny_probe_reports_probe_failure(tmp_path: Path) -> None:
    review_dir = tmp_path / "review"
    probe_path = tmp_path / "probe.json"
    _write_json(
        probe_path,
