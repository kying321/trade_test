...
class ReleaseOrchestratorTests(unittest.TestCase):
    ...
    def test_review_until_pass_success_promotes_baseline_snapshot(self) -> None:
        td = Path(tempfile.mkdtemp())
        self.addCleanup(lambda: shutil.rmtree(td, ignore_errors=True))
        d = date(2026, 2, 13)
        review_dir = td / "review"
        ...
        def _run_review(as_of: date) -> ReviewDelta:
            ...
            return ReviewDelta(
                ...
                defects=[],
                pass_gate=True,
                rollback_anchor="/tmp/review_anchor.yaml",
            )
        ...
        orch = ReleaseOrchestrator(
            ...
            health_check=lambda as_of, require_review: {"status": "healthy", "missing": []},
            stable_replay_check=lambda as_of, days: {"passed": True, "replay_days": 3, "checks": []},
            test_all=lambda **kwargs: {"returncode": 0, "summary_line": "error=none", "tests_ran": 1, "failed_tests": []},
            load_json_safely=lambda p: {},
        )

        out = orch.review_until_pass(as_of=d, max_rounds=1)
        self.assertTrue(bool(out.get("passed", False)))
        baseline = out.get("baseline_promotion", {}) if isinstance(out.get("baseline_promotion", {}), dict) else {}
        self.assertTrue(bool(baseline.get("promoted", False)))
        self.assertEqual(str(baseline.get("reason", "")), "ok")
        self.assertEqual(str(baseline.get("rollback_anchor", "")), "/tmp/review_anchor.yaml")

        promotion_json = review_dir / f"{d.isoformat()}_baseline_promotion.json"
        self.assertTrue(promotion_json.exists())
        promotion_payload = json.loads(promotion_json.read_text(encoding="utf-8"))
        self.assertTrue(bool(promotion_payload.get("promoted", False)))
        self.assertTrue(Path(str(promotion_payload.get("active_path", ""))).exists())
        self.assertTrue(Path(str(promotion_payload.get("snapshot_path", ""))).exists())
