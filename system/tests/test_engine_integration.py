...
class EngineIntegrationTests(unittest.TestCase):
    def _make_engine(self) -> tuple[LieEngine, Path]:
        ...
        cfg_data["validation"]["max_drawdown_max"] = 0.18
        cfg_data["validation"]["crypto_execution_friction_guard_enabled"] = False
    ...
    def test_run_review_cycle(self) -> None:
        ...
        ops_report = out.get("ops_report", {})
        self.assertIsInstance(ops_report, dict)
        self.assertIn("status", ops_report)
        self.assertIn(ops_report.get("status"), {"green", "yellow", "red"})
        self.assertNotIn("guard_loop_preflight", ops_report)
        baseline_from_heartbeat = (
            (
                (out.get("heartbeat", {}) or {}).get("guard_loop_preflight", {})
                if isinstance(out.get("heartbeat", {}), dict)
                else {}
            ).get("baseline_rollback_drill", {})
        )
        self.assertIsInstance(baseline_from_heartbeat, dict)
        self.assertIn("status", baseline_from_heartbeat)
        self.assertFalse(alert.exists())
    ...
    def test_run_backtest_uses_live_parameter_overrides(self) -> None:
        ...
        eng._core_symbols = lambda: ["510300", "510500"]  # type: ignore[method-assign]
    ...
    def test_run_backtest_crypto_runtime_params_are_bounded_and_use_warmup_ingest(self) -> None:
        import lie_engine.engine as eng_mod
        ...
        def _fake_ingest(start, end, symbols):
            captured["ingest_start"] = start
            captured["ingest_end"] = end
            captured["symbols"] = list(symbols)
            return pd.DataFrame(), None

        eng._run_ingestion_range = _fake_ingest  # type: ignore[method-assign]
        ...
        eng_mod.run_walk_forward_backtest = _fake_run_walk_forward_backtest  # type: ignore[assignment]
        try:
            bt_start = date(2026, 1, 1)
            bt_end = date(2026, 2, 13)
            eng.run_backtest(start=bt_start, end=bt_end)
        finally:
            eng_mod.run_walk_forward_backtest = original  # type: ignore[assignment]

        cfg = captured["cfg"]
        self.assertLess(float(getattr(cfg, "signal_confidence_min", 0.0)), 70.0)
        self.assertLess(float(getattr(cfg, "convexity_min", 0.0)), 2.5)
        self.assertLessEqual(int(getattr(cfg, "hold_days", 0)), 12)
        self.assertLessEqual(int(getattr(cfg, "max_daily_trades", 0)), 5)
        self.assertLess(captured["ingest_start"], date(2026, 1, 1))

    def test_run_backtest_crypto_prefers_factor_engine_when_legacy_undertrades(self) -> None:
        ...
        bars = pd.concat(
            [
                make_bars("BTCUSDT", n=320, start="2024-01-01", trend=0.05, seed=11, asset_class="cash"),
                make_bars("ETHUSDT", n=320, start="2024-01-01", trend=0.04, seed=12, asset_class="cash"),
                make_bars("BNBUSDT", n=320, start="2024-01-01", trend=0.03, seed=13, asset_class="cash"),
            ],
            ignore_index=True,
        )
        eng._run_ingestion_range = lambda start, end, symbols: (bars.copy(), None)  # type: ignore[method-assign]

        original_wf = eng_mod.run_walk_forward_backtest
        original_factor = eng_mod.run_crypto_factor_backtest
        ...
        def _fake_factor(**kwargs):
            result = BacktestResult(
                ...
                violations=0,
                positive_window_ratio=0.82,
                equity_curve=[{"date": kwargs["start"].isoformat(), "equity": 1.0}],
                by_asset={"cash": 0.002},
            )
            return SimpleNamespace(
                ...
                objective=0.11,
                dd_ok=True,
                params={"top_k": 3, "hold_days": 5, "risk_scale": 0.05},
                factor_weights={"mom_20": 0.5, "trend_ma20_60": 0.5},
                factor_ic_top=[],
            )

        eng_mod.run_walk_forward_backtest = _fake_wf  # type: ignore[assignment]
        eng_mod.run_crypto_factor_backtest = _fake_factor  # type: ignore[assignment]
        try:
            out = eng.run_backtest(start=date(2025, 1, 1), end=date(2025, 12, 31))
        finally:
            eng_mod.run_walk_forward_backtest = original_wf  # type: ignore[assignment]
            eng_mod.run_crypto_factor_backtest = original_factor  # type: ignore[assignment]

        self.assertEqual(int(out.trades), 120)
        manifest = tmp_root / "output" / "artifacts" / "manifests" / "backtest_2025-01-01_2025-12-31.json"
        payload = json.loads(manifest.read_text(encoding="utf-8"))
        self.assertEqual(str(payload.get("metrics", {}).get("selected_engine", "")), "crypto_factor")

    def test_run_backtest_crypto_failover_freeze_blocks_factor_engine(self) -> None:
        from types import SimpleNamespace
        import lie_engine.engine as eng_mod
        ...
        bars = pd.concat(
            [
                make_bars("BTCUSDT", n=320, start="2024-01-01", trend=0.05, seed=21, asset_class="cash"),
                make_bars("ETHUSDT", n=320, start="2024-01-01", trend=0.03, seed=22, asset_class="cash"),
            ],
            ignore_index=True,
        )
        eng._run_ingestion_range = lambda start, end, symbols: (bars.copy(), None)  # type: ignore[method-assign]

        state_path = tmp_root / "output" / "artifacts" / "crypto_factor_failover_state.yaml"
        state_path.parent.mkdir(parents=True, exist_ok=True)
        future_freeze_until = (date.today() + timedelta(days=10)).isoformat()
        state_path.write_text(
            yaml.safe_dump(
                {
                    "active": True,
                    "trigger_date": date.today().isoformat(),
                    "freeze_until": future_freeze_until,
        ...
        self.assertEqual(int(out.trades), 18)
        self.assertFalse(bool(called["factor"]))
        manifest = tmp_root / "output" / "artifacts" / "manifests" / "backtest_2025-01-01_2025-12-31.json"
        payload = json.loads(manifest.read_text(encoding="utf-8"))
        metrics = payload.get("metrics", {})
        self.assertEqual(str(metrics.get("selected_engine", "")), "event_wf_failover_freeze")
        self.assertTrue(bool(metrics.get("factor_failover_active", False)))

    def test_run_backtest_crypto_failover_triggers_on_consecutive_degraded(self) -> None:
        from types import SimpleNamespace
        import lie_engine.engine as eng_mod

        eng, tmp_root = self._make_engine()
        eng.settings.validation["crypto_factor_failover_window_runs"] = 3
        eng.settings.validation["crypto_factor_failover_profit_factor_min"] = 1.0
        eng.settings.validation["crypto_factor_failover_annual_return_min"] = 0.0
        eng.settings.validation["crypto_factor_failover_freeze_days"] = 2
        eng.settings.validation["crypto_factor_failover_lookback_days"] = 365

        bars = pd.concat(
            [
                make_bars("BTCUSDT", n=320, start="2024-01-01", trend=0.05, seed=31, asset_class="cash"),
                make_bars("ETHUSDT", n=320, start="2024-01-01", trend=0.03, seed=32, asset_class="cash"),
            ],
            ignore_index=True,
        )
        eng._run_ingestion_range = lambda start, end, symbols: (bars.copy(), None)  # type: ignore[method-assign]
        eng._recent_crypto_factor_runs = lambda as_of, lookback_days: [  # type: ignore[method-assign]
            {"end": "2025-10-31", "annual_return": -0.02, "profit_factor": 0.90, "max_drawdown": 0.04, "trades": 80},
            {"end": "2025-11-30", "annual_return": -0.01, "profit_factor": 0.95, "max_drawdown": 0.03, "trades": 90},
        ]
        ...
        self.assertEqual(int(out.trades), 0)
        manifest = tmp_root / "output" / "artifacts" / "manifests" / "backtest_2025-01-01_2025-12-31.json"
        payload = json.loads(manifest.read_text(encoding="utf-8"))
        metrics = payload.get("metrics", {})
        self.assertEqual(str(metrics.get("selected_engine", "")), "event_wf_failover_triggered")
        self.assertTrue(bool(metrics.get("factor_failover_triggered", False)))
        self.assertTrue(bool(metrics.get("factor_failover_active", False)))
        fail_state = yaml.safe_load((tmp_root / "output" / "artifacts" / "crypto_factor_failover_state.yaml").read_text(encoding="utf-8"))
        self.assertTrue(bool(fail_state.get("active", False)))

    def test_run_backtest_crypto_friction_guard_blocks_factor_engine(self) -> None:
        from types import SimpleNamespace
        import lie_engine.engine as eng_mod

        eng, tmp_root = self._make_engine()
        eng.settings.validation["crypto_execution_friction_guard_enabled"] = True
        eng.settings.validation["crypto_execution_friction_p95_bps_max"] = 35.0
        eng.settings.validation["crypto_execution_friction_depth_floor_usdt"] = 5_000_000.0
        eng.settings.validation["crypto_execution_friction_target_notional_usdt"] = 20_000.0
        eng.settings.validation["crypto_execution_friction_artifact_max_age_days"] = 2

        bars = pd.concat(
            [
                make_bars("BTCUSDT", n=320, start="2024-01-01", trend=0.05, seed=41, asset_class="cash"),
                make_bars("ETHUSDT", n=320, start="2024-01-01", trend=0.03, seed=42, asset_class="cash"),
            ],
            ignore_index=True,
        )
        eng._run_ingestion_range = lambda start, end, symbols: (bars.copy(), None)  # type: ignore[method-assign]
        eng._load_latest_crypto_execution_friction_summary = lambda: {  # type: ignore[method-assign]
            "path": str(tmp_root / "output" / "review" / "fake_friction.json"),
            "payload": {
                "as_of": "2025-12-31T00:00:00",
                "slippage_bps": {"notional_20000": {"p95": 62.0}},
                "liquidity": {"quote_volume_usdt": {"p50": 1_000_000.0}},
            },
        }
        ...
        payload = json.loads(manifest.read_text(encoding="utf-8"))
        metrics = payload.get("metrics", {})
        self.assertEqual(str(metrics.get("selected_engine", "")), "event_wf_friction_guard")
        self.assertTrue(bool(metrics.get("factor_friction_guard_blocked", False)))
    ...
    def test_engine_provider_profile_tushare_binance_hybrid(self) -> None:
        project_root = Path(__file__).resolve().parents[1]
        td = tempfile.TemporaryDirectory()
        self.addCleanup(td.cleanup)
        tmp_root = Path(td.name)
        ...
        cfg_data = yaml.safe_load((project_root / "config.yaml").read_text(encoding="utf-8"))
        cfg_data["paths"] = {"output": "output", "sqlite": "output/artifacts/lie_engine.db"}
        cfg_data["data"] = {"provider_profile": "tushare_binance_hybrid"}
        cfg_path = tmp_root / "config.yaml"
        cfg_path.write_text(yaml.safe_dump(cfg_data, allow_unicode=True), encoding="utf-8")

        eng = LieEngine(config_path=cfg_path)
        self.assertEqual(len(eng.providers), 1)
        self.assertEqual(getattr(eng.providers[0], "name", ""), "tushare_binance_hybrid")
